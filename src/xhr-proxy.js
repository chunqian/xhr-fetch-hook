...
export function proxy(proxy, realWindow = window) {
    if (realWindow.singletonAjaxHook) throw "Proxy already exists";
    return realWindow.singletonAjaxHook = new Proxy(proxy, realWindow);
}

export function unProxy(realWindow = window) {
    realWindow.singletonAjaxHook = null
    unHook(realWindow)
}
...
function Proxy(proxy, realWindow = window) {
